...
class ApiHelper {
  ...
  static Future<WeeklyWeather> getWeeklyForecast() async {
    await fetchLocation();
    final url = _constructWeeklyForcastUrl();
    final response = await _fetchData(url);
  ...
  static String _constructWeeklyForcastUrl() =>
  ...
  static Future<Map<String, dynamic>> _fetchData(String url) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        printWarnings(
          "Error Fetching data from $url ${response.statusMessage}",
        );
        throw Exception("Error fetching Data");
      }
    } catch (e) {
      printWarnings("Error Fetching data from $url : $e");
      throw Exception("Error fetching Data");
    }
  }
}
